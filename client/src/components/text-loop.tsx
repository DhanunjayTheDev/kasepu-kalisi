...
type TextLoopShape = "circle" | "infinity" | "arch" | "line" | "wave";
...
/**
 * React Bits' TextLoop, ported to TSX: a phrase repeated endlessly along an SVG
 * path (wave/circle/infinity/arch/line), driven by GSAP. Two copies of the text
 * are laid end-to-end on the path and shifted in lockstep so the loop has no seam.
 */
export default function TextLoop({
  ...
  const unit = useMemo(() => {
    // Non-breaking space (U+00A0) via fromCharCode, not a literal in source, so
    // no editor/linter mangles the byte and the gap can't wrap or collapse.
    const nbsp = String.fromCharCode(160);
    const base = uppercase ? String(text).toUpperCase() : String(text);
    const gap = separator ? `${nbsp}${separator}${nbsp}` : `${nbsp}${nbsp}${nbsp}`;
    return `${base}${gap}`;
